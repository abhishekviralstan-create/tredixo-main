...
const Home = () => {
  ...
  const categories = useMemo(() => {
    const blogCategories = allBlogs
      .map((blog) => blog.blogCategory?.trim())
      .filter(Boolean);

    return ['All Posts', ...new Set(blogCategories)];
  }, [allBlogs]);
  ...
  useEffect(() => {
    ...
    if (activeCategory !== 'All Posts') {
      updatedBlogs = updatedBlogs.filter(
        (blog) =>
          blog.blogCategory &&
          blog.blogCategory.toLowerCase() === activeCategory.toLowerCase()
      );
    }
          ...
          <>
                ...
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-sm transition-all duration-300 ${
                    activeCategory === category
                      ? 'bg-[#15181d] text-cyan-300 border border-cyan-400/30'
                      : 'bg-[#111317] text-white/60 border border-white/5 hover:text-white hover:border-white/15'
                  }`}
                >
                  {category}
                </button>
            ...
            {currentBlogs.length > 0 ? (
              ...
              <div className="mt-16 rounded-3xl border border-white/10 bg-[#0d0f13] p-10 text-center">
                ...
                <p className="mt-3 text-white/55">Try changing the category.</p>
              </div>
            )}
            ...
            {/* Animated Section Below Pagination */}
            <motion.section
              initial={{ opacity: 0, y: 30 }}
